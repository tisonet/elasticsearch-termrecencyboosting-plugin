...
public class RecencySimilarityProvider extends AbstractSimilarityProvider {
    private final RecencySimilarityDecorator similarity;

    @Inject
    public RecencySimilarityProvider(@Assisted String name, @Assisted Settings settings) {
        super(name);
        this.similarity = new RecencySimilarityDecorator(settings, new BM25Similarity());
    }

    public RecencySimilarityDecorator get() {
        return similarity;
    }
}